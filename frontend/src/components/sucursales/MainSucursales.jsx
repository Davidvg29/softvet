...
import CrearSucursales from './CrearSucursales';
import VerSucursales from './VerSucursales';
import EditSucursales from './EditSucursales';

const MainSucursales = () => {
  ...
  const cargarSucursales = async () => {
    try {
      const response = await axios.get(`${SUCURSALES}/ver`, { withCredentials: true });
  ...
  const sucursalesFiltrados = sucursal.filter((sucursal) =>
    sucursal.nombre_sucursal.toLowerCase().includes(busqueda.toLowerCase())
  );

  const borrarSucursales = async (id) => {
    ...
    try {
      ...
      const response = await axios.delete(`${SUCURSALES}/eliminar/${id}`, { withCredentials: true });
      ...
      Swal.fire({
        icon: 'error',
        title: 'Error',
        text: 'No se pudo eliminar la sucursal. Inténtalo nuevamente.',
        confirmButtonText: 'Aceptar',
        confirmButtonColor: '#6f42c1',
      });
    }
...
>
  ...
  >
    ...
    <tbody>
      {sucursalesFiltrados.length > 0 ? (
        sucursalesFiltrados.map((sucursal) => (
          <tr
          ...
          >
            ...
            >
              ...
              <Button
                ...
                onClick={() => handleOpenModal("ver", sucursal.id_sucursal)}
              >
                Ver
              </Button>
              ...
              <Button
                ...
                onClick={() => handleOpenModal("editar", sucursal.id_sucursal)}
              >
                Editar
              </Button>
              ...
              <Button
                ...
                onClick={() => borrarSucursales(sucursal.id_sucursal)}
              >
                Eliminar
              </Button>
            </td>
  ...
  >
    ...
    >
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
        {fromType === 'crear' && (
          <CrearSucursales onClose={handleCloseModal} onUpdate={cargarSucursales} />
        )}
        {fromType === 'ver' && <VerSucursales id_sucursal={sucursalId} />}
        {fromType === 'editar' && (
          <EditSucursales
            id_sucursal={sucursalId}
            onClose={handleCloseModal}
            onUpdate={cargarSucursales}
          />
        )}
      </div>
    </div>
